...
class FakeAuthService:
    def __init__(self) -> None:
        ...
        self.requested_base_urls: list[str | None] = []
        self.reset_payloads: list[object] = []

    async def forgot_password(  # noqa: ANN001
        self,
        payload,
        *,
        ...
        self.requested_base_urls.append(requested_base_url)
...
async def test_forgot_password_returns_generic_response() -> None:
    ...
    assert fake.requested_base_urls == [None]


@pytest.mark.asyncio
async def test_forgot_password_passes_request_origin_to_auth_service() -> None:
    fake = FakeAuthService()
    app.dependency_overrides[get_auth_service] = lambda: fake
    app.dependency_overrides[get_redis] = lambda: FakeRedis()

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="https://institution-staging.test") as client:
        response = await client.post(
            "/api/v1/auth/forgot-password",
            json={"email": "person@example.com"},
            headers={"Origin": "https://institution-staging.d3lrsnjzo6p8fc.amplifyapp.com"},
        )

    app.dependency_overrides.clear()
    assert response.status_code == 202
    assert fake.requested_base_urls == [
