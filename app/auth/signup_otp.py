...
class SignupOtpStore:
    """Redis-backed OTP and send-rate limits for email signup."""
    ...
    def _otp_key(self, email: str) -> str:
        return self._keys.otp(purpose=SIGNUP_OTP_PURPOSE, subject=email)

    def _send_rate_key(self, email: str) -> str:
        return self._keys.rate_limit(bucket="signup_otp_send", identifier=email)

    async def store_otp(self, email: str, code: str) -> None:
        ttl = self._settings.signup_otp_ttl_seconds
        await self._redis.set(self._otp_key(email), hash_otp_code(code), ex=ttl)
...
"""
    ...
    async def enforce_send_rate(self, email: str) -> None:
