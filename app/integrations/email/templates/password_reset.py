...
def render_password_reset(context: PasswordResetContext) -> TransactionalEmailContent:
    ...
    if context.reset_url:
        safe_url = html_escape(context.reset_url)
        html_body = render_html(
            ...
            content=(
                ...
                f'<p style="margin:0 0 16px;font-size:14px;line-height:1.6;word-break:break-all;">'
                f'<a href="{safe_url}" style="color:#1f6f68;">{safe_url}</a></p>'
                f'<p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:#5d6966;">'
                f"This link expires in {context.ttl_minutes} minutes and can only be used once.</p>"
